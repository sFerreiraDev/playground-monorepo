...
describe(`NumberUtils`, () => {
  describe(`firstFreeIndex`, () => {
    ...
    it(`[], .05, 1 => 0`, () => {
      expect(NumberUtils.firstFreeIndex([], 0.05)).toBe(0.05);
    });
    it(`[0, 1, 3], 0, 1 => 2`, () => {
    ...
    it(`[-.01, 0, .01, .03], 0, .01 => .02`, () => {
      expect(NumberUtils.firstFreeIndex([-0.01, 0, 0.01, 0.03], 0, 0.01)).toBe(0.02);
    });
    it(`[-.01, 0, .01, .03], .03, .01 => .04`, () => {
      expect(NumberUtils.firstFreeIndex([-0.01, 0, 0.01, 0.03], 0.03, 0.01)).toBe(0.04);
    });
  });
});
